...
recognizers, their focus node and their keyboard handling — and own **nothing**
...
Simple, matches Flutter's `builder` idiom, and **costs a rebuild per hover**. Use
it when the *structure* of the visual changes with the state — a checkmark that
appears, a spinner that replaces a label.
...
The state change **retargets an interpolation a render object is already
observing**. Hovering rebuilds nothing at all; it repaints one node.

This is the path that milestones 7 and 8 paid for, and it is what the walkthrough
uses for every control.

  </TabItem>
</Tabs>

<Aside type="tip" title="Reading statesOf subscribes to the scope, not the controller">
`ComponentScope::statesOf(context)` makes your element depend on the *scope*,
which only changes when a different component wraps you — almost never. The
subscription to the controller itself is one you make explicitly, which is what
lets you route it to a render object instead of to a rebuild.
</Aside>
...
<CardGrid>
  ...
  <Card title="A progress fill" icon="bars">
    ...
    makes it a fill rather than a zoom — and it is a paint operation, so no
    layout runs.
  </Card>
</CardGrid>
...
In practice it does not need to. `enabled` and `selected` are **configuration**,
not interaction — they are known at build time:
...
Fill, corner radius, border colour, border width. That is all — no gradients, no
shadows, no background images, no blur. A "sleek modern" look therefore has to be
built out of restraint: hairline borders, small radii, and contrast carried by
the border rather than by a fill. That constraint is on the
[gaps list](/fltr/project/gaps-and-rework/), and it is the single most visible
one.
</Aside>
