...
at the end of the phase with a pointer store and a generation bump. Chunks are
retained, so the steady state allocates nothing.

That only works if resetting is free, which means widgets must be trivially
destructible. `Arena::create<T>` enforces it:
...
`createOwning` is deliberately more verbose, so a widget that owns heap memory
reads as the exception.

<Aside type="caution" title="What this costs you">
Anything a widget refers to by pointer must outlive the widget, because the
widget will not keep it alive. That covers `ScrollController`, `FocusNode`,
`WidgetStatesController`, `ValueListenable<T>`, and the `string_view` inside a
`Text`. See [memory and lifetimes](/fltr/architecture/memory-and-lifetimes/).
</Aside>
...
assignment, and a `memcpy` for the plain-old-data configurations that are the
norm.
...
carried by value, so a ref whose arena has since been reset can still be compared
for reconciliation without being dereferenced.

`Element::updateChild` reads:

```cpp
if (!widget.stale()) child->update(widget);
```

Re-emitting a stale ref retains a subtree without rebuilding it, which is how
[`AnimatedSwitcher`](/fltr/architecture/overlay/) keeps a subtree alive while it
animates away.

<Aside type="danger" title="The sharp edge">
A stale ref reaching `inflate`, meaning it is used to build something new rather
than to match something existing, is a contract violation. With `FLTR_CHECKS=1`
it traps; with checks off it is undefined behaviour. In practice this only
happens if you cache a `WidgetRef` across frames, so don't.
</Aside>
...
<CardGrid>
  <Card title="Keyed children survive reordering" icon="approve-check">
    ...
    so moving it leaves the state where the position was.
  </Card>
