...
`kPanSlop = 36`, `kPrecisePointerSlop = 1`, `kPrecisePointerPanSlop = 2`,
`kLongPressTimeout = 0.5`, `kDoubleTapTimeout = 0.3`, `kDoubleTapSlop = 100`. A
drag picks between the two pairs by the pointer's device kind, as Flutter's
`computeHitSlop` does; everything else holds every pointer to the finger's
figure, which is what keeps a click that wobbles a click.
