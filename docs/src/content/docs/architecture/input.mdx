...
`kPanSlop = 36`, `kPreciseSlop = 1`, `kPrecisePanSlop = 2`, `kLongPressTimeout =
0.5`, `kDoubleTapTimeout = 0.3`, `kDoubleTapSlop = 100`. A drag picks between the
two pairs by the pointer's device kind; everything else holds every pointer to
the finger's figure.
