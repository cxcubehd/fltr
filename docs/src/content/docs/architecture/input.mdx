...
nothing. Keeping them apart is deliberate — each one has a different answer to
"what is under the pointer" and a different idea of when to ask.
...
delta into a local one. That limitation is recorded and accepted.
</Aside>
...
routes everything afterwards **by pointer id** to the recognizers that registered
for it.
...
2. A recognizer must keep receiving events after the pointer has left the region
   it started in — a drag that wanders off the slider still belongs to the
   slider.
...
2. **`close(pointer)` runs immediately after.** If only one member joined, it wins
   right there — the common case costs nothing.
...
5. **`hold` / `release`** defer the sweep — a double-tap recognizer holds the
   arena while it waits for the second tap.
...
When a member is removed — because its render object is being destroyed — the
arena deliberately does not promote the next candidate. Firing a game callback
...
actually wants one. A hover-only region holds **zero**, and `recognizerCount()`
is asserted in the test suite to prove it.
...
event rate; fitting gives one a fling can be built on.
...
kept sorted with an insertion sort, deliberately — the library sort allocates a
temporary buffer, and the no-per-frame-allocation rule does not allow that.

## Hover is a diff, not a gesture

`MouseTracker` builds the newly-hovered set, **swaps it in before any callback
runs**, then fires `exit()` on what left and `enter()` on what arrived. Doing it
in that order means a callback that rebuilds the tree cannot corrupt the
iteration.
...
returns `true`. There is no contest and no deferral — this is what a browser does
when a scrolled-to-the-end list lets the page scroll instead.

The `bool` is the whole point: `false` means no UI wanted it, so your game may
zoom the camera with it. `dispatchKey` answers the same question for keys, and
the walkthrough uses both to hand the ship its controls back when a menu closes.
