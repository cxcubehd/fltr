...
The design assumes something is animating almost always, and that several things
are animating at once. That constrains everything below.
...
- An unstarted ticker holds no subscription at all, so "an animation in a hidden
  panel costs nothing" is `activeTickerCount()`, a number the tests assert.
...
A shorter journey takes proportionally less time, so reversing from 0.3 takes 30%
of the duration. Pointer enter and exit arrive mid-animation constantly, and this
is what keeps the result continuous rather than suddenly slow.
</Aside>

`statusChanges()` is a separate channel from the value channel. Logic that waits
for an animation to settle should not be woken sixty times a second to be told it
has not.
...
unset by default, which is the continuous choice: a different reverse curve
...
The new interval begins at the value on screen, not at the old tween's origin.
Repeated interruption accumulates no error: hover in, hover out, hover in again
mid-fade a hundred times, and the value stays continuous.
...
<CardGrid>
  <Card title="Cheap: render-attached" icon="rocket">
    ...
    `observeForPaint`s it. A tick invalidates one render object, rebuilds nothing
  ...
  <Card title="Expensive: layout-attached" icon="warning">
    ...
    `AnimatedAlign`'s doc comment calls it "the one whose frames are not free,
...
and runs the driver from zero. Only the configuration change rebuilds. Every tick
after it invalidates exactly one render object, because what the built widget
hands down is the animated value itself rather than a snapshot of it.
...
integrated), `ClampingScrollSimulation` (Android's power law) and
`BouncingScrollSimulation` (iOS: friction until it leaves the range, then a
spring).

These sit alongside the driver rather than replacing it. A driver is normalised
0..1 over a duration; a simulation is unbounded in value and settles by
tolerance. Scrolling runs on the simulations, UI transitions on the driver.
...
2. **Assert `activeTickerCount()`.** An animation in an unmounted or muted subtree
   should hold no subscription.
