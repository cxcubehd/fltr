...
focus nodes, no key handlers and no subscriptions — `KeyboardBinding::handlerCount()`
is literally zero. That is not an optimisation; it is what makes pointer-only a
...
3. **Prefer one whose extent overlaps on the other axis** — the "band". Moving
   right along a row of buttons should not jump to something diagonally below
...
object. **What it costs:** you cannot have a traversal group that is not also a
focus scope, so "these five buttons wrap around among themselves but share the
enclosing scope's focus memory" is not expressible.
</Aside>
...
then copied again into the `State` when the configuration is adopted — because a
key arrives long after that arena was released.
...
consumer. `OwnedFocusNode` handles both, and uses a `Subscription` as a
**liveness token**: if a consumer destroys a node while a component still names
it, the component reads it as *gone* rather than dangling.
...
`AnchorLink`. It is the closest thing fltr has to a weak pointer, and it costs
two pointer stores.

## Components get keyboard activation for free

Focus landed **before** the component layer, deliberately. That is why every
`RawButton`, `RawToggle` and `RawSlider` has keyboard activation, a focus state
in its published `WidgetStates`, and gamepad navigation designed in rather than
retrofitted — and why turning all of it off is one flag rather than a different
widget:
