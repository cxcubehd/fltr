...
draw outside their parent's subtree. That is what the overlay is for. Dialogs and
menus additionally need to survive their own removal long enough to fade out,
which is a separate and much more expensive mechanism.

## The overlay

```mermaid
flowchart TD
  OV["RenderOverlay"] --> BASE["child 0, the base:<br/>alone decides the size"]
...
it is laid out exactly as it would have been without the overlay, so inserting a
...
// Reading the overlay is an ambient read, so it happens during a build.
OverlayState* overlay = Overlay::of(context);
// Inserting happens later, from a callback.
...
The host widget that builds an entry refers to it by an `std::int64_t` rather
than by address, because an entry can be destroyed part-way through the build
...
Size is resolved at layout; position is resolved at paint. Paint is the one phase
where every ancestor offset is final, so that is where the anchor's rectangle in
root space is knowable. `keepOnScreen` clamps a menu opened near an edge.

<Aside type="caution" title="An anchored entry follows a repaint, not a movement">
If something moves the anchor without laying it out, which is what scrolling
does, the entry does not know. Call `AnchorLink::markMoved()` in that case. Tying
the entry to layout instead would have made every scroll frame relayout the
overlay.
</Aside>

## Exit animations

This is the most expensive item in fltr per unit of user-visible benefit, so it
is worth knowing exactly what it does.
...
`AnimatedSwitcher` retains the outgoing element, not the widget, by re-emitting
its stale `WidgetRef` on every build. Reconciliation reads:
...
3. **Letting go happens in the build.** The element is dropped on the first build
   after its presence reaches zero, rather than from inside the animation
   callback, which would tear down a tree from within a notification it is
   walking.
...
apart by key. Two presences with the same key is what makes a stale ref reach
`inflate`: a contract violation with checks on, undefined behaviour with them
off. Key by what the page is, not by what widget it happens to be.
</Aside>
...
The presence value is a `ValueListenable<float>` running from 0 to 1. It is handed
to a render object, so a fade costs one repaint per frame and no builds.
...
**Drag and drop.** The overlay is the piece a drag ghost needs, and it exists. The
drop machinery (draggables, drop targets, and the hit testing between them) was
never built.
