---
title: The frame
description: What drawFrame does, in order, and why each of those orderings matters.
---

import { Aside, Steps } from '@astrojs/starlight/components';

`WidgetBinding::drawFrame(float seconds)` is the only call into fltr's frame.
Dispatching input and pushing state happen outside it, at whatever rate your
platform produces events.
...
input. `needsFrame` earns its place when submission itself is expensive, such as
a retained-mode backend that would otherwise re-walk the list.
...
The drain is capped at 32 passes in every build, so the failure mode is one stale
frame rather than a frozen game. `FrameStats::layoutPasses` exposes the count. A
healthy frame is 0 or 1.
...
- `markNeedsLayout` refuses to run during the paint phase, because painting must
  not dirty layout.
- `paintWithContext` requires the paint phase, so nothing is painted outside it.
...
`BuildOwner::buildCount()` and `TickerRegistry::activeTickerCount()` sit
alongside them. Those two are how you check that hovering costs no rebuilds and
that a hidden animation costs nothing.
