...
- the parent passed `parentUsesSize = false`, so it has already committed to not
  caring;
- the incoming constraints are tight, so there is only one legal size;
- the node is `sizedByParent()`, so its size is a function of constraints alone.
...
2. **Divide what is left.** Each flexible child gets `freeSpace * flex /
   ...
   `remainingFlex == d.flex` its extent is just `remainingSpace`, so children
   fill the row exactly instead of leaving a sub-pixel gap.
...
|---|---|
| Cross axis unbounded and `CrossAxisAlignment::Stretch` | Degrades to loose, since there is nothing to stretch to |
...
```
A `Scrollable` lays its child out with the scroll axis unbounded, which is why a
`Column` inside one must not itself be unbounded on that axis.
  </TabItem>
