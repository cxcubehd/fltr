---
title: Scrolling
description: Scrolling as a paint operation, physics as a stateless policy, activities as an enum, and the non-lazy limitation stated plainly.
---

import { Aside, Card, CardGrid } from '@astrojs/starlight/components';

The single most important thing about scrolling in fltr: **it is a paint
operation, and that is measured, not asserted.**
...
scrolling re-records one clip and one `DrawList` reference** — not the content.
Hit testing applies the same `paintOffset()`, so what you click is what you see.
...
`BouncingScrollPhysics` (iOS), `NeverScrollPhysics` (disabled *without losing the
position*). Nothing allocates a physics object per scrollable; you point at one.

Overscroll is simply **what the boundary refused**: `applyBoundaryConditions`
returns the part of a delta it would not accept, and the stretch is rendered
through the existing render-attached transform path.
...
| `Overscroll` | Wraps a scrollable. Stretches the content at the ends. |
...
controller is also watched, not just the position — an indicator wrapping a
scrollable is built first, before the scrollable below it has a position at all,
and has to tolerate that rather than assume it away.
</Aside>
...
This was a deliberate choice, and the reasoning is worth keeping: position,
physics, activities, controller, wheel, overscroll and the scrollbar are all
**identical** whether or not children are lazy. Adding laziness later requires
build-during-layout — a scoped exemption from phase separation plus keeping the
build arena open across layout — but it does not require revisiting any of the
above.
