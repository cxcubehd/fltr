...
Unconditional. `refresh` measures, `setSurface` compares before it invalidates,
and `syncGraphics` only publishes what changed, so a frame in which the window
did not move costs two comparisons.
...
Polling is also the step's one hazard, and a press is where it shows. raylib
samples the mouse button once a frame and reports the edge it finds between two
samples, so a press and a release that both land inside one frame cancel out and
the click is never seen. A click is over in a few tens of milliseconds, which is
about as long as a frame, so the browser's events are queued as they arrive and
...
The position travels with the event because a click is long over by the frame
that reports it. On a desktop the queue is filled from raylib's own edges, which
a window redrawing in a millisecond does not miss -- the difference is one
function, and `pumpPointer` above it dispatches a list either way.

`trackedPointer` is the same field a `mousemove` writes, which is the point: the
hover, the press and the wheel notch all read one position, taken from the page
in the pixels the surface is measured in. raylib's own answer arrives through
emscripten's GLFW shim, which scales it by the ratio between the window size it
was last told about and the canvas box -- a second opinion about a number the
page already knows exactly, and one that disagrees for a frame whenever the
canvas is resized. `mouseleave` is on the same handler, because a pointer that
left the page is hovering nothing and nothing else will say so.
...
Before `buildAndLayout`, so a value the game pushed this frame is visible to the
build that follows it. Pushing after would show it one frame late.
...
The entire framework-facing surface of the loop. Everything else on this page is
yours.
...
After `drawFrame`, because hover is resolved inside it. Reading the cursor before
would apply last frame's answer.
...
The game and the UI are drawn by different code into the same framebuffer.
...
3. **Input before simulation.** The ship acts on this frame's keys.
...
<Aside type="note" title="One caveat">
Seven steps, one of which calls into fltr; the other six are things a game already
does. Nothing checks the ordering for you, though. Calling `applyCursor` before
`drawFrame` compiles, runs, and is subtly wrong.
</Aside>
