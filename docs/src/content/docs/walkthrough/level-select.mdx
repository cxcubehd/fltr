...
That is deliberate and it is what you want here: a locked card is drawn over the
scroll content, and a click on it must not fall through to whatever is behind. If
you want the other behaviour, wrap it in `IgnorePointer` — both exist, and the
distinction is why.
</Aside>
...
`Scrollable` owns the position and lends it to the controller — and the two clear
each other's pointer on the way out, so either outliving the other is inert rather
than dangling.
...
`Scrollbar` and `Overscroll` both watch the controller **and** the position,
because a controller has no position until the scrollable below it has built —
and an indicator that wraps a scrollable is always built first.
...
The scroll layer is complete for a menu: physics, a fling, a fading draggable
scrollbar, an overscroll stretch, and focus traversal that scrolls the next card
into view by the minimum amount.

Two findings, both already known: no lazy children, and no grid — a 5×2 layout of
levels would have to be a `Column` of `Row`s with the arithmetic done by hand.
</Aside>
