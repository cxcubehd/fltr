...
Eight asteroid fields, later ones locked -- more than the viewport can show, so
the list genuinely scrolls. This is the step that exercises scrolling,
...
Every card is built and laid out whether or not it is visible. A handful is fine;
a few thousand would not be.
...
Where `takeWheelDelta` comes from is the platform's business, and the two
platforms do not agree on what a wheel even is. A desktop reports notches, and
reading them per axis is enough:
...
A browser reports what the device measured, in CSS pixels, and raylib does not
pass that on: its web backend reads the wheel through emscripten's GLFW shim,
which quantizes every event to at least a whole notch and hands the horizontal
delta over unscaled beside it. A trackpad reports a few pixels at a time, so
through that shim a gentle swipe arrives as a notch a frame. The DOM event is
listened for directly instead, and drained once a frame:
