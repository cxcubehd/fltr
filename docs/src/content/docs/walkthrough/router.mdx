...
Four wrappers, each doing exactly one thing:
...
There is deliberately no `Shortcuts` at the root. A shortcut is a focus node,
and a key only reaches it on its way up from whatever is focused -- so a
root-level Escape works right up until the focus is nowhere, which is exactly
what a HUD looks like. Escape is app-wide, so it lives one layer lower, in the
platform code that already asks the UI what it declined:

```cpp title="docs/demo/src/platform/input.cc"
const bool taken = binding.dispatchKey(KeyEvent{...});
...
A shortcut *scoped to a screen* is still the right tool, and the main menu uses
one for S and Q.
