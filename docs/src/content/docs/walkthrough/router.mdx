...
fltr has no navigator, no routes and no screen stack — deliberately, because a
game engine already has one. What it has instead is enough to build one in about
forty lines, and this is that.
...
| `Shortcuts` | Escape and Backspace, wherever the focus is |
...
`AnimatedSwitcher` decides two children are the same child when their **type and
key match** — the same rule reconciliation uses everywhere. Every screen here is
wrapped in a `Watch<bool>`, so they all have the same type. Without the key they
would update in place and the transition would never run.
</Aside>
...
It is deliberately the same shape as `AnimatedValue<T>`: a `ValueListenable` that
subscribes to a source and republishes. Owning one is a `State`'s job, because a
render object holds a pointer to it.
...
animations consume literally nothing — and resume exactly where they stopped,
because a ticker carries a per-frame delta rather than an absolute clock.
...
Nothing was missing, and the framework's refusal to ship a navigator looks correct
for a game engine.

The one real cost: `MappedValue` had to be invented before any of it could stay on
