...
The ship flies now. What matters on this page is not the physics — it is the
**seam**: how a simulation running at 60 Hz feeds a UI without either side
...
**continuously**, which is exactly the load the reactivity layer should be shown
under — a readout that only moves when you press a button proves nothing.
...
moved — and something always moves. Six separate observables mean the score
readout hears about the score and nothing else.
</Aside>
...
One `lround` is the difference between rebuilding the speed readout **sixty times
a second** and rebuilding it when the digits change. fltr cannot make that decision
for you — it does not know how you are going to render the number.
...
also makes it `sizedByParent` — an unconditional relayout boundary. A HUD cluster
resizing never reaches the view.
...
`AbsorbPointer` would do the opposite — swallow the click. That is what a disabled
control over live content wants, and it is not what this wants.
</Aside>
...
## The expensive counter, on purpose
...
Both are on the same screen, deliberately. Shoot an asteroid and watch the debug
readout: `builds` climbs for 0.45 s while the score rolls, then goes flat. The
...
The reactivity layer holds up under a continuously-changing simulation. Six
observables, pushed unconditionally at 60 Hz, and the only thing that rebuilds is
the one readout whose digits actually changed — plus the count-up, which cannot
avoid it.

The finding is the count-up itself: **there is no animated text**. A rolling number
is a rebuild per frame, and no arrangement of the existing pieces avoids it.
</Aside>
