...
Most specific last. **Focus composes rather than replaces** — it recolours the
border of whichever surface won, so a focused *and* hovered control still looks
hovered.
...
That distinction is the whole trick. `statesOf` creates an *ambient* dependency on
the scope — which effectively never changes — and the subscription to the
controller is one we make **explicitly**, so we choose where it goes.
...
Let the **surface** carry hover and press — interaction, animated, zero rebuilds.
Let the **label** carry enabled and selected — configuration, known at build time,
no animation needed.
...
The component layer is the framework's strongest idea. Splitting behaviour from
appearance at the `WidgetStates` boundary meant one 130-line file gave every
control in the demo animated hover, press, focus and disabled states — for the
cost of one repaint per frame of animation and **zero** rebuilds.

Measured, not claimed: the smoke test asserts `buildCount()` stays put while
hovering.
</Aside>
