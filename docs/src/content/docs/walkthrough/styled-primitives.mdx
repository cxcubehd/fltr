...
The two guards are the demo's own `:focus-visible`. Something is always focused --
...
affordances, and a keyboard walking the focus should not leave a second control
lit under a cursor nobody has moved. The input layer publishes which device is
driving, the theme carries it, and exactly one of the two highlights answers:
...
Tab is what asks for the keyboard, and the only key that does: Escape, the arrows
and an activator all mean something on their own, so none of them is read as a
request to start walking the focus.
