...
rule for everything a widget refers to by pointer: `ScrollController`, `FocusNode`,
`WidgetStatesController`, `AnchorLink`, and this.
...
Read it during the build, keep the pointer, use it from a callback. Any "open a
dialog from a button" flow has this shape.
...
The scrim is inside the animated subtree rather than a second overlay entry. One
subtree in, one subtree out, one animation, and the scrim's fade stays in step
with the panel's.
...
`StackFit::Expand` makes the scrim fill the overlay, which fills the base child, so
it covers the whole HUD without anyone computing a size.
...
panel is the result screen rather than a paused field, so Escape leaves for the
level list instead of resuming.
...
The base child alone decides the overlay's size, because an overlay must never
change the layout of what it overlays however large an entry is. Entries paint
after the base and, because hit testing walks children in reverse, are tested
before it,
which is what makes the scrim swallow clicks aimed at the HUD.

<Aside type="note" title="Exit animations cost nothing structurally">
No keep-alive flag, no dismissal callback, no second tree. A `WidgetRef` carrying
its own type and key is what made retaining an outgoing subtree fall out of
reconciliation instead of being bolted onto it.

One rough edge: `Overlay::of` is a build-only read, so an app that opens panels
from callbacks has to stash the `OverlayState*` during a build. Three lines here,
and not obvious the first time.
</Aside>
