...
raylib's own `DrawRectangleRounded` takes a single roundness as a fraction of the
shorter side, which cannot express four independent radii, and its rounded line
routine leaves a visible seam where the ring meets itself. So the geometry is
built here instead: one outline, four radii, scaled down together when
neighbours would overlap.
...
The fill is a triangle fan over that outline. The border is one closed triangle
strip between the outline and the same outline inset by the border width -- a
ring with no seam to leave a gap or a double-blended overlap at:
...
void Backend::strokeRounded(Rect bounds, const BorderRadius& radius, float width, ::Color color) {
  ...
  buildOutline(innerOutline_, /* bounds inset by thickness */, inset(corners, thickness), steps);
...
<Aside type="caution" title="Winding is not decoration">
raylib enables backface culling and treats counter-clockwise as front-facing, so
an outline wound the other way draws nothing at all. Both outlines here are wound
the way raylib winds its own shapes, and the strip pairs them inner-first for the
same reason.
</Aside>
