...
trivially-copyable command array; a consumer walks it. That is the entire
contract, so what follows is a switch statement and some stacks. Nothing is
...
There are three push kinds and one `Pop`. The obvious implementation — compare
stack depths and guess which one closed — is **wrong**, because pushes nest but do
not alternate:
...
Reset, walk, and make sure the scissor is off on the way out — the game draws into
the same framebuffer next frame.
...
4×4. The mapping is mechanical, and the zeros are load-bearing — get one wrong and
everything shears.
...
Honouring it would need stencil work, so the demo drops the corners — visible only
where something actually overflows a rounded container.
...
`DrawRRect` is one command carrying fill **and** border, so both come out of one
case. The zero-roundness branch is not an optimisation — raylib's rounded
routines misbehave at roundness zero.
...
3. **One thing that is fine but undocumented** — `PushOpacity` having no analogue
   in an immediate-mode renderer, so the backend must fold alpha into every colour
   it emits.

</Steps>

<Aside type="tip" title="What this step proved">
"No graphics API coupling" is literally true. The framework's entire output is an
array of PODs, and translating it needed no cooperation from fltr at all — no
registration, no virtual calls back into the consumer, no lifetime negotiation.
</Aside>
