...
## Where the line sits, and where the box ends

Two decisions that a text service makes for every panel above it.

The first is where the baseline goes inside a line box. Hanging the glyphs from
the top and leaving all the leading underneath -- the obvious implementation --
is what makes every padded button in a UI look bottom-heavy. This one centres
the capitals in the line box instead:
...
The second is what the paragraph reports as its own height. A font's box
reserves room for ascenders and descenders whether or not the text has any, so a
label above a number ends up further from the bottom of its panel than from the
top. The box is trimmed to the text itself -- cap top to baseline -- and the
padding around it becomes the whole of the space that shows:
...
Only the outer edges are trimmed: between lines the full line height still
applies, so a wrapped paragraph reads exactly as before.

<Aside type="note" title="Both ratios come from the font">
`ascentRatio_` and `capRatio_` are read from the font's own 'H' at startup.
fltr's placeholder service hardcodes the first and says so; a real one should
